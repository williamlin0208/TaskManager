...
const NewWork = () => {
  ...
  const showMessage = (str) => {
    if (str == "ok") {
      return Alert.alert(
        "Success",
        "New work has been posted on the bulletin.",
        [{ text: "Ok", onPress: () => navigation.navigate("Bulletin") }]
      );
    } else {
      return Alert.alert("Failed", str, [{ text: "Back", style: "cancel" }]);
    }
  };
  ...
  const checkValid = () => {
    ...
    else if (parseInt(reward) < 0 || parseInt(reward) > 100000)
      return "Reward should be set between 0 and 100000.";
